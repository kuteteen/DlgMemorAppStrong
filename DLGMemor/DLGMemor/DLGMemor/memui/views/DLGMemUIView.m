...
@property (nonatomic) UIButton *btnMotifyArray;
...
- (void)initMotifyArrayButton {
    UIButton *btn = [UIButton buttonWithType:UIButtonTypeSystem];
    btn.translatesAutoresizingMaskIntoConstraints = NO;
    [btn setTitle:@"MotifyArray" forState:UIControlStateNormal];
    [btn setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    [btn addTarget:self action:@selector(onMotifyArrayTapped:) forControlEvents:UIControlEventTouchUpInside];
    [self.vMore addSubview:btn];
    
    NSDictionary *views = @{@"reset":self.btnReset, @"btn":btn};
    NSArray *ch = [NSLayoutConstraint constraintsWithVisualFormat:@"H:[reset][btn]" options:0 metrics:nil views:views];
    [self.vMore addConstraints:ch];
    NSArray *cv = [NSLayoutConstraint constraintsWithVisualFormat:@"V:|[btn]|" options:0 metrics:nil views:views];
    [self.vMore addConstraints:cv];
    
    self.btnMotifyArray = btn;
}
...
- (void)onMotifyArrayTapped:(id)sender {
    NSLog(@"self.chainCount==%d",self.chainCount);
    if (!(self.chainCount > 0 && self.chainCount <= MaxResultCount)){
        return;
    }
    DLGMemUIViewCell *cell = [self.tvResult cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:0]];
    NSMutableArray  *array  =   [NSMutableArray array];
    for (int i=0; i<self.chainCount; i++)
    {
        search_result_t result = chainArray[i];
        NSString *address = [NSString stringWithFormat:@"%llX", result->address];
        [array addObject:address];
    }
    if ([self.delegate respondsToSelector:@selector(DLGMemUIModifyArrayValue:address:type:)]) {
        [self.delegate DLGMemUIModifyArrayValue:[cell.tfValue text] address:array type:[self currentValueType]];
    }
}
